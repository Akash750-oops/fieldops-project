...
export default function AlertBanner({
  ...
  const fetchAlerts = async () => {
    try {
      const response = await fetch("http://localhost:8000/jobs/redispatch-alerts/all");
      if (response.ok) {
        const data = await response.json();
        setAlerts(data);
      }
    } catch (err) {
      console.warn("Failed to fetch initial redispatch alerts", err);
    }
  };
  ...
  const handleDismissLocal = (jobId: number) => {
