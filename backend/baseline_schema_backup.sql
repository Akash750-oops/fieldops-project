...
\restrict c6qO0zmckLPaQGBE4aRFMe72vgIOP8xS8JzhWncpShZwbIMDZv9N8TMsrR0jgMi
...
CREATE TABLE public.ai_brand_safety_rules (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_ai_brand_safety_category CHECK (((category)::text = ANY (ARRAY[('COMPETITOR'::character varying)::text, ('POLITICAL'::character varying)::text, ('OFF_BRAND'::character varying)::text, ('BLOCKED_PHRASE'::character varying)::text]))),
    CONSTRAINT ck_ai_brand_safety_match_type CHECK (((match_type)::text = ANY (ARRAY[('WORD'::character varying)::text, ('PHRASE'::character varying)::text]))),
    CONSTRAINT ck_ai_brand_safety_severity CHECK (((severity)::text = ANY (ARRAY[('INFO'::character varying)::text, ('WARNING'::character varying)::text, ('ERROR'::character varying)::text, ('CRITICAL'::character varying)::text])))
);
...
CREATE TABLE public.communication_channel_configurations (
    ...
    CONSTRAINT ck_communication_channel_state CHECK (((state)::text = ANY (ARRAY[('ENABLED'::character varying)::text, ('DISABLED'::character varying)::text, ('EMERGENCY_ONLY'::character varying)::text])))
);
...
CREATE TABLE public.customer_preference_audits (
    ...
    correlation_id character varying(100),
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_audit_actor_source CHECK (((actor_source)::text = ANY (ARRAY[('CUSTOMER'::character varying)::text, ('ADMIN'::character varying)::text, ('SYSTEM'::character varying)::text]))),
...
CREATE TABLE public.notifications (
    ...
    CONSTRAINT valid_status CHECK (((status)::text = ANY (ARRAY[('UNREAD'::character varying)::text, ('READ'::character varying)::text, ('DISMISSED'::character varying)::text])))
);
...
CREATE TABLE public.organizations (
    ...
    CONSTRAINT ck_organizations_plan CHECK (((subscription_plan)::text = ANY (ARRAY[('FREE'::character varying)::text, ('STARTER'::character varying)::text, ('PROFESSIONAL'::character varying)::text, ('ENTERPRISE'::character varying)::text]))),
    CONSTRAINT ck_organizations_status CHECK (((status)::text = ANY (ARRAY[('ACTIVE'::character varying)::text, ('SUSPENDED'::character varying)::text, ('DELETED'::character varying)::text])))
);
...
\unrestrict c6qO0zmckLPaQGBE4aRFMe72vgIOP8xS8JzhWncpShZwbIMDZv9N8TMsrR0jgMi
